...
router.post('/', async (req, res, next) => {
    try {
        const {tree, location, heightFt, groundCircumferenceFt, createdAt, updatedAt} = req.body
        const findTree = await Tree.findOne({where: {name: tree}})
...
router.delete('/:id', async (req, res, next) => {
    try {
        const treeId = req.params.id
        if (treeId) {
            const findTree = await Tree.findOne({where:{id: treeId}})
